...
from server_utils.others import parse_txt_and_mood_simple, wait_for_human_response
...
if __name__ == '__main__':
    ...
    while True:
        # time.sleep(0.5)
        human_message = states.get_human_message()
        if human_message is None or len(human_message) == 0:
            print("no human message!\n")
            time.sleep(2)
            continue
        # memory.human_prefix = 'Environment'
        ipt = "communication from human: "
        ipt += json.dumps(human_message, ensure_ascii=False)
        print("inputs: ", ipt)
        ...
        while True:
            try:
                ...
                print(e)
        txt_mood = parse_txt_and_mood_simple(output)
